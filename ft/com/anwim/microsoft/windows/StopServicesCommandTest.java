...
import com.anwim.microsoft.windows.services.StopServicesCommand;

public class StopServicesCommandTest extends TestCase {
	@Override
	...
	public void testExecuteForMissingServiceZZTop() {
		...
		try {
			...
			Assert.assertNotNull("Expecting not to stop ZZTop service.", result
	...
	public void testExecuteForInvalidServerName() {
		...
		try {
			...
			Assert.assertNotNull(
					"Expecting not to stop DoesNotMatter service on ZZTop.", result
							.containsKey(Command.ERROR));
		} catch (Exception e) {
			fail("This should not throw an exception: " + e.getMessage());
		}
