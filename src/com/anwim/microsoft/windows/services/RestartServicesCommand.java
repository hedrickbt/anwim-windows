...
public class RestartServicesCommand extends Command {
			...
			if (response.getStdErr().size() > 0) {
				...
				if (response.stdOutContains(" FAILED ")) {
					...
					if (success) {
						...
						result.put(Command.ERROR, "Output does not indicate the service was restarted.  The word Running was not found.");
					}
				}
			}
		} catch (Exception e) {
